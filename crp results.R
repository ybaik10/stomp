...
casecontrols %>% filter(as.Date(date_enrolled) > as.Date("2018-05-24") & as.Date(date_enrolled) < as.Date("2019-09-20") & is.na(crp_result_quantifiable.factor)) %>% select(participanttype, crp_specimen_date, date_enrolled, why_no_crp.factor)

# missing CRPs to look for:
casecontrols %>% filter(is.na(why_no_crp.factor) & as.Date(date_enrolled) > as.Date("2018-05-24") & as.Date(date_enrolled) < as.Date("2019-09-01") & is.na(crp_result_quantifiable.factor)) %>% 
